...
function buildSearchParams({ query = "", page = 1, resultsPerPage = 4 }) {
...
async function postYokSearch({ query = "", page = 1, resultsPerPage = 4 }) {
  const response = await fetch(YOK_SEARCH_URL, {
    ...
    body: buildSearchParams({ query, page, resultsPerPage })
  });
...
async function searchRealTheses({ query = "", page = 1, resultsPerPage = 4 }) {
  const html = await postYokSearch({ query, page, resultsPerPage });
...
async function getRecentRealTheses(offset = 0, limit = 4) {
...
export async function getFeed(cursor = 0, limit = 4) {
  const cacheKey = buildCacheKey("feed", `${cursor}:${limit}`);
  const cached = getCached(cacheKey);
  ...
  try {
    const payload = await getRecentRealTheses(cursor, limit);
...
export async function searchTheses(query = "") {
  ...
  const remote = await fetchFromPlaywrightServer("/search", { q: query });
  ...
  try {
    const results = await searchRealTheses({ query, page: 1, resultsPerPage: 8 });
    if (results.length > 0) {
  ...
  try {
    ...
    const fallbackResults = filterThesesByQuery(recentPool, query).slice(0, 24);
  ...
  return setCached(cacheKey, results.map(normalizeSample));
}
...
export async function getDisciplineFeed(disciplineId = "") {
  const normalizedId = normalizeWhitespace(disciplineId);
  const cacheKey = buildCacheKey("discipline-feed", normalizedId);
  const cached = getCached(cacheKey);
